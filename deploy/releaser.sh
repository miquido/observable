...
sed -e "s/\"version\": \"${VERSION}\",/\"version\": \"${NEW_VERSION}\",/g" composer.json > composer.json.tmp && mv composer.json.tmp composer.json

# Create release commit and tag
git add CHANGELOG.md composer.json
git commit -m "chore(release): ${NEW_VERSION} :tada:
$(conventional-changelog)
"
git tag "v${NEW_VERSION}"

# Push commit and tag
git remote add authorized https://travis:${GH_TOKEN}@github.com/miquido/observable.git
